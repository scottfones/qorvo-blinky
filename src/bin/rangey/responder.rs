...
use qorvo_blinky::ultrawideband::transport::{
    EventLine, MAX_PHY_PACKET_SIZE, abort_receive, receive_frame, wait_tx_done,
};
...
pub async fn respond_and_range(
    ...
    let response_tx_programmed = delayed_tx_time(poll_rx_ticks, REPLY_DELAY_DTU);
    let response_frame = MessageType::encode_response(msg_id);

    let Some(response_tx_instant) = DwInstant::new(response_tx_programmed) else {
        defmt::error!("responder: delayed tx instant out of range");
        return Ok(dw);
    };

    let mut dw_sending = dw.send_raw(
        &response_frame,
        SendTime::Delayed(response_tx_instant),
        &ultrawideband::radio_config(),
    )?;

    let response_tx_ticks = wait_tx_done(&mut dw_sending, event_line).await?.value();
    ...
    .await
    {
        Ok(Ok((msg_length, final_rx_instant))) => {
            ...
            let final_rx_ticks = final_rx_instant.value();
            if let Some(DecodedMessage::Final {
                msg_id: final_id,
                time_tx_poll: time_tx_poll_ticks,
                time_rx_response: time_rx_response_ticks,
                time_tx_final: time_tx_final_ticks,
            }) = buffer.get(..msg_length).and_then(MessageType::decode)
            {
                if final_id == msg_id {
                    let time_round1 = dtu_elapsed(time_rx_response_ticks, time_tx_poll_ticks);
                    let time_reply1 = dtu_elapsed(response_tx_ticks, poll_rx_ticks);
                    let time_round2 = dtu_elapsed(final_rx_ticks, response_tx_ticks);
                    let time_reply2 = dtu_elapsed(time_tx_final_ticks, time_rx_response_ticks);
                    let time_of_flight =
                    ...
                    defmt::info!(
                        "responder: seq={=u8} dist_mm={=i64} ema_mm={=i64}",
                        final_id,
                        millimeters,
                        ema_mm
                    );
                    ...
                    defmt::warn!(
                        "responder: final seq mismatch (got {=u8}, want {=u8})",
                        final_id,
                        msg_id
                    );
                }
            } else {
                defmt::warn!("responder: final decode failed");
            }
            Ok(dw_ready)
        }
        Ok(Err(e)) => {
