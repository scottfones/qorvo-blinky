...
pub async fn initiator_exchange(
    ...
    let poll_tx_ticks = wait_tx_done(&mut dw_sending, event_line).await?.value();
    ...
    let (dw, response_rx_ticks) = match embassy_time::with_timeout(
        RESPONSE_TIMEOUT,
        receive_frame(&mut dw_receiving, event_line, buffer),
    )
    .await
    {
        Ok(Ok((msg_length, response_rx_instant))) => {
            let dw_ready = dw_receiving.finish_receiving().map_err(|(_dw, e)| {
                defmt::error!("initiator: response finish_receiving: {}", e);
                e
            })?;

            match buffer.get(..msg_length).and_then(MessageType::decode) {
                Some(DecodedMessage::Response { msg_id: rsp_id }) if rsp_id == msg_id => {
                    leds.on_rx();
                    (dw_ready, response_rx_instant.value())
                }
                _ => {
                    defmt::warn!(
    ...
    let final_tx_programmed = delayed_tx_time(response_rx_ticks, REPLY_DELAY_DTU);
    let final_tx_predicted = predicted_tx_time(final_tx_programmed, u64::from(tx_antenna_delay));
    let final_frame =
        MessageType::encode_final(msg_id, poll_tx_ticks, response_rx_ticks, final_tx_predicted);

    let Some(final_tx_instant) = DwInstant::new(final_tx_programmed) else {
    ...
    let mut dw_sending = dw.send_raw(
        &final_frame,
        SendTime::Delayed(final_tx_instant),
